...
with tab3:
    ...
    bar_fig = go.Figure(data=[go.Bar(x=ZONES, y=moisture_values, marker_color=bar_colors, text=[f"{m}%" for m in moisture_values], textposition='auto')])
    bar_fig.update_layout(yaxis=dict(range=[0, 100], title="Moisture Level (%)"), xaxis=dict(title="Farm Zones"), margin=dict(l=0, r=0, t=30, b=0), height=300)
...
with tab5:
    ...
    with col_gauge:
        batt_fig = go.Figure(go.Indicator(mode="gauge+number", value=st.session_state.drone_battery, title={'text': "Battery Charge"}, gauge={'axis': {'range': [0, 100]}, 'bar': {'color': "lime" if st.session_state.drone_battery > 20 else "red"}, 'steps': [{'range': [0, 20], 'color': "maroon"}, {'range': [20, 100], 'color': "#232323"}]}))
